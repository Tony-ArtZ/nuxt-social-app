...
<template>
  <div class="px-2 bg-neu-green-light grid place-items-center">
    <section
      ...
      <label for="image" class="relative w-full">
        <img v-if="imagePreview" :src="imagePreview" class="object-cover w-full mb-6 aspect-square rounded-xl" />
        <button v-if="imagePreview" @click.prevent="removeImageHandler"
          class="absolute w-8 h-8 text-black border-4 border-black rounded-full bg-neu-green right-1 top-1 aspect-square">
          <Icon name="ic:sharp-close" size="16" />
        </button>
        <div
      ...
      <button @click.prevent="createPost"
        class="p-6 text-lg font-bold border-4 border-black shadow-neu-black bg-neu-green w-44 rounded-xl">
